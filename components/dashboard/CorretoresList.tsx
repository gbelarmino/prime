...
import { getCorretoresListUrl, isApiConfigured } from "@/lib/api-config";
...
export function CorretoresList() {
  const router = useRouter();
  const menuRef = useRef<Menu>(null);
  const [selectedRow, setSelectedRow] = useState<CorretorApiResponse | null>(null);

  const buildUrl = useCallback(
    (page: number, size: number, q: string) => getCorretoresListUrl(page, size, q),
    []
  );
  ...
  const actionItems = [
    { 
      ...
      command: () => {
        if (selectedRow) router.push(`/dashboard/corretores/edit?id=${selectedRow.id}`);
      }
    },
    { separator: true },
    { 
  ...
  const {
    ...
    pageData,
    fetchError,
  } = usePaginatedSpringList<CorretorApiResponse>({
    buildUrl,
    pageSize: 10,
    fallbackErrorMessage: "Não foi possível carregar os corretores.",
  ...
  const nameBodyTemplate = (rowData: CorretorApiResponse) => {
    return (
  ...
  const emailBodyTemplate = (rowData: CorretorApiResponse) => {
  ...
  const creciBodyTemplate = (rowData: CorretorApiResponse) => {
    return (
      <div className="flex items-center gap-2">
        <BadgeCheck size={14} className="text-blue-400" />
        <span className="text-white/80 font-mono text-sm">{rowData.creci}</span>
      </div>
    );
  };

  const imobiliariaBodyTemplate = (rowData: CorretorApiResponse) => {
    return (
      <div className="flex items-center gap-2 text-white/60 text-sm">
        <Building2 size={14} className="text-blue-400/60" />
        <span className="truncate max-w-[200px]">{rowData.imobiliariaRazaoSocial || "Sem Imobiliária"}</span>
      </div>
    );
  };

  const actionBodyTemplate = (rowData: CorretorApiResponse) => {
    return (
  ...
  const header = (
    <div className="flex flex-col md:flex-row justify-between items-center gap-4 py-2">
      ...
      <div className="text-sm text-white/40">
        <span className="text-white font-bold">{totalRecords}</span> corretores encontrados
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl ">
      ...
      >
        <Column field="nome" header="Corretor" body={nameBodyTemplate} className="min-w-[250px]" />
        <Column field="creci" header="CRECI" body={creciBodyTemplate} />
        <Column field="imobiliariaRazaoSocial" header="Imobiliária" body={imobiliariaBodyTemplate} />
