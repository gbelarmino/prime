...
import { SmsConversaListItem } from "./SmsConversaListItem";
import { cn } from "@/lib/utils";

type Props = {
  conversas: SmsConversa[];
...
export function SmsInboxColumn({
  conversas,
  selectedId,
  flashConversaId,
  loading,
  ...
  const filtradas = useMemo(() => {
    ...
    if (buscaDeferred) {
      list = list.filter((c) => {
        const hay = [c.tituloExibicao, c.clienteNome, c.telefone, c.ultimaMensagemPreview]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
    ...
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-[var(--wa-desk-surface)]">
      <div className="space-y-2 border-b border-white/10 px-3 py-2.5">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md border border-sky-500/35 bg-sky-950/30 px-2 py-1 text-[10px] font-medium text-sky-200/90 transition-colors hover:border-sky-400/50 hover:bg-sky-900/40"
              title="Iniciar conversa SMS"
            >
        ...
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-black/30 px-2.5 py-1.5 text-sm text-white placeholder:text-white/30 focus:border-sky-400/40 focus:outline-none"
        />
      ...
      <ul role="listbox" aria-label="Conversas SMS" className="min-h-0 flex-1 overflow-y-auto">
