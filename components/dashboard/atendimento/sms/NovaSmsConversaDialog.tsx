...
export function NovaSmsConversaDialog({ visible, onHide, onEnviar }: Props) {
  const [clientes, setClientes] = useState<ContratanteOption[]>([]);
  const [cliente, setCliente] = useState<ContratanteOption | string | undefined>(
    undefined,
  );
  ...
  async function onSelectCliente(opt: ContratanteOption) {
    ...
    if (phones.length === 1) setTelefone(phones[0]);
      ...
      setErro("Cliente sem celular — digite o telefone abaixo");
    }
  }
  ...
  async function handleEnviar() {
    ...
    try {
      const selected =
        cliente && typeof cliente === "object" && "id" in cliente
          ? cliente
          : undefined;
  ...
  return (
    <DashboardDialog
      ...
      pt={{
        ...
        footer: {
          className: "border-t border-white/[0.06] bg-transparent px-5 py-4",
        },
      }}
      footer={
    ...
    >
      <div className="flex flex-col gap-3 text-sm text-white/85">
        <p className="text-[12px] text-white/45">
          Escolha um cliente cadastrado ou digite o telefone. A mensagem inicial é
          opcional — texto livre, sem template.
        </p>
        ...
        <label className="flex flex-col gap-1">
          ...
          <input
            ...
            className="rounded-lg border border-white/10 bg-black/30 px-2.5 py-2 font-mono text-sm text-white placeholder:text-white/30 focus:border-sky-400/40 focus:outline-none"
          />
        </label>
        ...
        <label className="flex flex-col gap-1">
          ...
          <InputTextarea
            ...
            placeholder="Texto livre do SMS…"
          />
          <span
            className={`text-[11px] ${
              mensagem.length > 160 ? "text-amber-300/90" : "text-white/35"
            }`}
          >
