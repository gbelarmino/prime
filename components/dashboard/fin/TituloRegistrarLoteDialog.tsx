...
type TituloRegistrarLoteDialogProps = {
  visible: boolean;
  onHide: () => void;
  titulos: TituloCobranca[];
  resultado?: TituloRegistrarLoteResult | null;
  onConfirm: () => void;
  loading?: boolean;
};
...
export function TituloRegistrarLoteDialog({
  visible,
  onHide,
  titulos,
  resultado,
  onConfirm,
  loading = false,
}: TituloRegistrarLoteDialogProps) {
  const grupos = useMemo(() => agruparPorContrato(titulos), [titulos]);
  const confirmando = !resultado;

  return (
    <DashboardDialog
      header={confirmando ? "Registrar títulos em lote" : "Resultado do registro"}
      visible={visible}
      onHide={onHide}
      className={cn(
        ...
        <div className="flex justify-end gap-3">
          {confirmando ? (
            <>
              ...
              <button
                type="button"
                disabled={loading || titulos.length === 0 || titulos.length > REGISTRO_LOTE_MAX}
                onClick={onConfirm}
              ...
              >
                {loading
                  ? "A registrar…"
                  : `Registrar ${titulos.length} título${titulos.length === 1 ? "" : "s"}`}
              </button>
            </>
          ) : (
            <button
      ...
      {confirmando ? (
        <div className="flex flex-col gap-4">
          <p className="text-sm text-white/50">
            Os títulos selecionados serão registrados no convênio de cada empreendimento. Cada
            registro é independente — falhas parciais não revertem os já emitidos.
          </p>
          {titulos.length > REGISTRO_LOTE_MAX ? (
            <p className="rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-sm text-rose-200/90">
              Selecione no máximo {REGISTRO_LOTE_MAX} títulos por operação.
            </p>
          ) : null}
            ...
            {grupos.map((grupo) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-white/75">
                  Parcelas:{" "}
                  {grupo.titulos.map((t) => t.numeroParcela).join(", ")}
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-4">
          <p className="text-sm text-white/60">
            <span className="font-semibold text-emerald-300">{resultado.registrados}</span> de{" "}
            {resultado.total} registrado(s)
            {resultado.falhas > 0 ? (
              <>
                {" "}
                ...
                {resultado.itens.map((item) => {
                  ...
                  return (
                    <tr key={item.tituloId} className="bg-white/[0.02]">
                      ...
                      >
                        {item.sucesso ? "Emitido" : item.mensagem ?? "Erro"}
                      </td>
                    </tr>
                  );
                })}
