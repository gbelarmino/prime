...
const IGPM_SERIE_COMPLETA_DESDE = "2015-01";
...
export function IgpmIndiceList() {
    ...
    async (background = false) => {
      ...
      try {
        ...
        const [lista, ultimoMes] = await Promise.all([
          ...
          finService.getIndiceIgpmUltimo().catch(() => null),
        ]);
        setRows(lista);
        setUltimo(ultimoMes);
      } catch (e) {
        toast.error(e instanceof Error ? e.message : "Falha ao carregar índices IGP-M.");
        setRows([]);
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const sincronizar = async () => {
    setSyncing(true);
    try {
      const result = await finService.sincronizarIndicesIgpm();
      if (result.status === "SUCESSO") {
        toast.success(
          `IGP-M sincronizado (histórico completo): ${result.registrosNovos} novos, ${result.registrosAtualizados} atualizados.`,
        );
        ...
        toast.error(result.erro ?? "Falha ao sincronizar IGP-M.");
      }
      await load(true);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Não foi possível sincronizar com o BCB.");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const percentualBody = (valor: number | null | undefined) => {
    ...
    return (
      <IgpmIconCell icon={<Percent size={14} className="text-violet-400/60" />}>
  ...
  const acum12Body = (valor: number | null | undefined) => {
    if (valor == null) return <span className="text-white/20">—</span>;
    return (
      <IgpmIconCell icon={<Percent size={14} className="text-amber-400/60" />}>
  ...
  const numeroIndiceBody = (valor: number | null | undefined) => {
    if (valor == null) return <span className="text-white/20">—</span>;
    return (
      <IgpmIconCell icon={<Hash size={14} className="text-violet-400/60" />}>
        <span className="font-mono text-sm tabular-nums tracking-tight text-white/60">
  ...
  const sincronizadoBody = (valor: string | null | undefined) => {
    ...
    return (
      <IgpmIconCell icon={<Clock size={14} className="text-violet-400/60" />}>
  ...
  const header = (
    <div className="flex flex-col gap-4 py-2">
        ...
        <div className="flex flex-wrap items-center gap-3 md:justify-end">
          ...
          {admin ? (
            <button
              ...
              disabled={syncing}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-500/30 bg-violet-600/20 px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-violet-200 transition-all hover:bg-violet-600/30 disabled:pointer-events-none disabled:opacity-50"
            >
      ...
      <p className="text-xs text-white/35">
        Fonte: BCB SGS (séries 189 e 13522). A variação acumulada em 12 meses é a referência contratual
        para reajuste de parcelas. No primeiro arranque da API, a série desde jan/2015 é carregada do
        BCB; o job mensal atualiza os meses recentes. Use &quot;Sincronizar BCB&quot; (admin) para forçar
        atualização.
      </p>
    </div>
  );
    ...
    <div className="flex flex-col gap-5 px-4">
      {ultimo ? (
        <div className="grid gap-3 sm:grid-cols-3">
          <IgpmSummaryCard label="Último mês" value={formatMesReferencia(ultimo.ano, ultimo.mes)} />
          <IgpmSummaryCard label="Variação mensal" value={formatPercentual(ultimo.variacaoMensal)} />
          <IgpmSummaryCard
      ...
      <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 shadow-2xl">
        <DataTable
          ...
          emptyMessage="Nenhum índice IGP-M sincronizado. Peça a um administrador para sincronizar com o BCB."
...
function IgpmIconBox({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-violet-500/10 bg-violet-500/10 text-violet-400 transition-all duration-300 group-hover:bg-violet-600 group-hover:text-white">
      {children}
    </div>
  );
}

function IgpmIconCell({
  icon,
  children,
}: {
  icon: ReactNode;
