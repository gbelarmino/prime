...
function LinhaSimulacao({ item }: { item: IpcaSimulacaoParcela }) {
  ...
  return (
    <>
      ...
      <table className="w-full text-left text-xs">
        ...
        <tbody className="divide-y divide-white/[0.06] text-white/70">
          {simulacao.map((item) => (
            <LinhaSimulacao key={item.parcela} item={item} />
          ))}
        </tbody>
      </table>
    </>
...
export function TitulosIpcaSimulacaoWorkspace() {
  ...
  const [simulacao, setSimulacao] = useState<IpcaSimulacaoParcela[]>([]);
  ...
  useEffect(() => {
    ...
    const periodo = periodoIpcaParaSimulacao(primeiraParcela.vencimento, parcelaAtual);

    void finService
      .listIndicesIpca(periodo)
      .then((indices) => {
        if (cancelled) return;
        setSimulacao(
          simularParcelasIpca({
      ...
      .catch(() => {
        if (!cancelled) {
          toast.error("Falha ao carregar índices IPCA para a simulação.");
          setSimulacao([]);
        }
      })
      .finally(() => {
  ...
  }, [pesquisado, titulos, contexto, parcelaAtual, primeiraParcela]);
      ...
      {pesquisado ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-[2rem] border border-dashed border-white/10 bg-white/[0.02] px-6 py-20 text-center">
          ...
          <p className="text-sm text-white/40">
            Selecione empreendimento, quadra e lote e clique em Pesquisar para comparar títulos
            emitidos com a simulação IPCA.
          </p>
        </div>
      )}
    </div>
