...
import { MessageSquare, RefreshCw, Search, UserCheck, Users, X } from "lucide-react";
import {
  smsService,
...
function celularCell(row: ContratanteListItem) {
  const raw = row.telefoneCelular1?.trim();
  if (!raw) return dashboardCellText("—");
  return dashboardCellMono(formatPhoneDisplay(raw));
}

export function SmsTesteEventos() {
  ...
  const loadClientes = useCallback(async (q: string) => {
    ...
    try {
      const page = await smsService.listContratantes(0, 200, q);
  ...
  const handleDisparar = async (codigo: string, descricao: string) => {
    ...
    try {
      const result = await smsService.dispararTesteEvento(
        codigo,
        selected.map((c) => c.id),
      );
      if (result.enfileirados > 0) {
        toast.success(
          `${descricao}: ${result.enfileirados} enfileirado(s), ${result.ignorados} ignorado(s).`,
        );
      } else {
        toast.warning(`${descricao}: nenhum SMS enfileirado. Verifique gatilhos, TextBee e celulares dos clientes.`);
      }
  ...
  return (
    <WhatsAppSectionShell
      ...
      description="Seleccione os clientes que receberão o SMS e dispare cada evento como em produção (gatilho + modelo + fila). Requer TextBee configurado."
      actions={
        <button
          type="button"
          onClick={() => {
            ...
            <div className={cn("min-h-0 flex-1 pt-1", DASHBOARD_DATATABLE_INSET_SHELL_CLASS)}>
              ...
              >
                ...
                <Column
                  ...
                  style={{ width: "32%" }}
                />
                <Column field="cpf" header="CPF" body={cpfCell} style={{ width: "30%" }} />
              </DataTable>
            </div>
          </section>
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 flex-1 space-y-1.5">
                      ...
                      {pronto && (ev.codigo === "COBRANCA_PARCELA" || ev.codigo === "REGUA_COBRANCA") ? (
                        <p className="pt-1 text-xs leading-relaxed text-white/40">
                          O teste escolhe automaticamente um título compatível com a situação do evento
                          (em aberto, vence hoje ou em atraso).
                        </p>
                      ) : null}
                    </div>
                    <Button
