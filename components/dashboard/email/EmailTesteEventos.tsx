...
import { Mail, RefreshCw, Search, UserCheck, Users, X } from "lucide-react";
import {
  emailService,
  EMAIL_TITULO_COBRANCA_TESTE_ID,
...
function emailCell(row: ContratanteListItem) {
  const raw = row.email?.trim();
  if (!raw) return dashboardCellText("—");
  return dashboardCellMono(raw);
}
...
export function EmailTesteEventos() {
  ...
  const loadClientes = useCallback(async (q: string) => {
    ...
    try {
      const page = await emailService.listContratantes(0, 200, q);
  ...
  const handleDisparar = async (codigo: string, descricao: string) => {
    ...
    try {
      const result = await emailService.dispararTesteEvento(
        codigo,
        selected.map((c) => c.id),
      );
      if (result.enfileirados > 0) {
        ...
        toast.warning(`${descricao}: nenhum e-mail enfileirado. Verifique gatilhos, SMTP e e-mails dos clientes.`);
      }
  ...
  return (
    <WhatsAppSectionShell
      ...
      description="Seleccione os clientes que receberão o e-mail e dispare cada evento como em produção (gatilho + modelo + fila). Requer SMTP configurado."
      actions={
        <button
          type="button"
          onClick={() => {
            ...
            <div className="min-h-0 flex-1 pt-1">
              {selected.length === 0 ? (
                <div className="flex h-full min-h-[300px] flex-col items-center justify-center rounded-xl border border-dashed border-white/10 bg-white/[0.02] px-8 py-10 text-center">
                  <Mail size={32} className="mb-3 text-white/15" />
