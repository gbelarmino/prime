...
import { emailService, type EmailGatilho, type EmailTemplate } from "@/lib/email-service";
...
export function EmailGatilhos() {
  ...
  const handleUpdate = async (evento: string, templateId: string | null, ativo: boolean) => {
    ...
    setSaving(evento);
    try {
      const saved = await emailService.saveGatilho({
        ...existing,
        evento,
        template: template ? { id: template.id!, nome: template.nome, assunto: template.assunto } : null,
        ativo: ativo ? "S" : "N",
      });
      setGatilhos((prev) => {
  ...
  return (
    <WhatsAppSectionShell
      ...
      description="Canal independente do WhatsApp. Active só os eventos que deseja enviar por e-mail."
      actions={
        <button
          type="button"
          disabled={fetching}
