...
export function CrmFunilGatilhos() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [gatilhos, tpls, etapasList] = await Promise.all([
        fetchFunilGatilhos(),
  ...
  const templatesPorEvento = useMemo(() => {
    const map = new Map<string, WhatsAppTemplate[]>();
    for (const row of rows) {
      map.set(
        row.eventoCodigo,
        templates.filter((t) => t.codigoEventoCatalogo === row.eventoCodigo),
      );
    }
    return map;
  }, [rows, templates]);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 md:px-8">
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/45">
          Notificações WhatsApp quando ocorrem eventos do pipeline (ex.: lead mudou de etapa). Crie modelos em
          WhatsApp com categoria do evento correspondente. Para avisar o corretor, cadastre o celular no cadastro do
          corretor.
        </p>
        <button
          type="button"
      ...
      <div className="space-y-4">
        {rows.length === 0 ? (
          <p className="rounded-2xl border border-white/10 bg-white/[0.02] px-5 py-8 text-center text-sm text-white/50">
            Nenhum evento de lead configurável. Aplique a migração{" "}
            <span className="font-mono text-white/70">12_funil_gatilho_whatsapp.sql</span>.
          </p>
        ) : null}

        {rows.map((row) => {
          const tplOptions = templatesPorEvento.get(row.eventoCodigo) ?? [];
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="flex flex-col gap-1.5">
                  ...
                  <Dropdown
                    ...
                    optionValue="id"
                    placeholder={
                      tplOptions.length ? "Selecione um modelo" : "Crie um modelo com este evento"
                    }
                    disabled={isSaving}
                    onChange={(e) => {
                      const tpl = tplOptions.find((t) => t.id === e.value);
                      const next = {
                        ...row,
